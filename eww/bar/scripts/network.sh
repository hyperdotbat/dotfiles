  ...
  case "${command}" in
    primary)
      ...
      function get_primary() {
        ...
        # elif [ "${ethernet_status}" == "połączono" ]; then primary="ethernet"
        # elif [ "${wifi_status}" == "połączono" ]; then primary="wifi"
        else primary="None"
        fi
    ...
    wifi)
      ...
      function get_wifi_network() {
        ...
        elif [ "${signal}" -ge 10   ] && [ "${signal}" -lt 30 ]; then icon="󰤟"
        elif [ "${signal}" -ge 31   ] && [ "${signal}" -lt 60 ]; then icon="󰤢"
        elif [ "${signal}" -ge 61   ] && [ "${signal}" -lt 99 ]; then icon="󰤥"
...
function connman() {
  case "${command}" in
    primary)
      function get_primary() {
        ethernet_status=`connmanctl technologies | grep -A 4 "Type = ethernet" | awk '/Connected/{print $3}'`
        wifi_status=`connmanctl technologies | grep -A 4 "Type = wifi" | awk '/Connected/{print $3}'`
        primary=""

        echo $wifi_status
        if [ "${ethernet_status}" == "True" ]; then primary="ethernet"
        elif [ "${wifi_status}" == "True" ]; then primary="wifi"
        else primary="None"
        fi
      ...
      get_primary
      # connmanctl monitor | while read -r line; do get_primary; done
    ;;
